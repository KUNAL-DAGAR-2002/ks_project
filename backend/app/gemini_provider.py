...
class GeminiProvider:
    ...
    async def extract(self, path: str, mime: str, document_type: str, existing_products: list[str] | None = None) -> dict:
        ...
        catalogue=json.dumps((existing_products or [])[:300],ensure_ascii=False)
...
Existing product catalogue: {catalogue}
For every sales or purchase row, first compare the OCR name with this catalogue. Ignore casing, extra spaces, hyphen/dash variants (-, --, —), harmless punctuation, singular/plural packaging words, and small spelling mistakes. If it is clearly the same product, product_name MUST be the exact catalogue string. Never merge different pack sizes, and never merge loose with packed. If there is no safe catalogue match, standardize product_name as Brand + Product + Variant + Size, using normal spaces and units kg, g, L or ml.
